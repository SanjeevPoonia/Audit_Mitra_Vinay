...
class QaUploadArtifactScreen extends StatefulWidget
{
  final String sheetID,paramID,subParamID,tempAuditID,auditID,paramIndex;
  var selectedLocation;
  QaUploadArtifactScreen(this.sheetID,this.paramID,this.subParamID,this.tempAuditID,this.auditID,this.paramIndex,this.selectedLocation);
  ArtifactState createState()=>ArtifactState();
}

class ArtifactState extends State<QaUploadArtifactScreen>
{
                    ...
                    child: Container(
                      ...
                      child: ElevatedButton(
                        ...
                        onPressed: () async {
                          ...
                          final data = await Navigator.push(context, MaterialPageRoute(builder: (context)=>MarkAttendanceScreen()));

                          if(data!=null)
                          {
                            imageList.add(data);
                            // Process the picked image, for example, display it or upload it to a server
                            print('Selected Image Path: ${data.path}');
                            setState(() {
  ...
  uploadImage() async {
    ...
    List<Map<String,String>> uploadedImageList=[];
    for(int i=0;i<imageList.length;i++)
    {
      ...
      var response = await dio.post(AppConstant.appBaseURL + "v2/storeArtifact",
          data: formData);
      ...
      if (response.data['status'] == 1) {
        var formData = {
          ...
          "parameter_id": widget.paramID,
          "sub_parameter_id": widget.subParamID,
          "imagePath":imageList[i].path.toString()
        };
        AppModel.setRebuttalData(formData);

        List data=(response.data['data'] as List?) ??[];
        for(int i=0;i<data.length;i++){
          String filePath=data[i]['file']?.toString()??"";
          if(filePath.isNotEmpty){
            Map<String,String> map={
              "imagePath":filePath,
              "name":filePath.split("/").last,
            };
            uploadedImageList.add(map);
  ...
  saveImage() async {
    ...
    for(int i=0;i<imageList.length;i++)
    {
      ...
      Map<String,String> map={
            "imagePath":imageList[i].path.toString(),
            "name":imageList[i].path.split("/").last,
          };
          uploadedImageList.add(map);
      ...
      if(i==imageList.length-1)
      {
        Toast.show("Images saved successfully!",
            duration: Toast.lengthLong,
            gravity: Toast.bottom,
            backgroundColor: Colors.green);
        Navigator.pop(context,{
